...
if os.path.exists('/storage/extSdCard'):
    ...
    sdir = '/storage/extSdCard/lp2017/'
else:
    db = SqliteDatabase('/usb/termux/termux-git/db/lessonplan2010.db', **{})
    sdir = '/usb/termux/termux-git/lp2017/'
...
def masuknota():
    print "Masuk Nota\n"
    nota = raw_input("Masukkan nota dalam LP 2017: \n")
    u = Lessonplan2017.select().where(Lessonplan2017.note ==\
                                  '-').order_by(Lessonplan2017.date)
    for i in u:
        print i.id, i.date, i.tingkatan, i.timestart, i.theme, i.topic

    selectlpid = raw_input("Masukkan no LP 2017\n: ")
    query = Lessonplan2017.update(\
                note=nota).\
               where(Lessonplan2017.id == selectlpid)
    query.execute()
    n = Lessonplan2017.select().where(Lessonplan2017.id == selectlpid)
    for a in n:
...
def viewdate():
    ...
    u = Lessonplan2017.select().where(Lessonplan2017.date ==\
                                  hb).order_by(Lessonplan2017.timestart)
...
def viewweek():
    ...
    u = Lessonplan2017.select().where(Lessonplan2017.week ==\
                                  week).order_by(Lessonplan2017.date)
...
def searchlptheme():
    ...
    tema = raw_input("Masukkan tema: \n")
    u = Lessonplan2017.select().where(Lessonplan2017.theme.contains(tema))
    for i in u:
...
def searchlptopic():
    ...
    topik = raw_input("Masukkan topik: \n")
    u = Lessonplan2017.select().where(Lessonplan2017.topic.contains(topik))
    for i in u:
...
def viewemptylp():
    lp = Lessonplan2017.select().where(Lessonplan2017.theme == '-').\
            order_by(Lessonplan2017.timestart).\
            order_by(Lessonplan2017.date)
    for l in lp:
...
def pindahbankkelp2017():
    ...
    lp2017baru = Lessonplan2017.select().where(Lessonplan2017.id == selectlpid)
...
def deletelp2017id():
    lpid = raw_input("Masukkan id Lesson 2017 LP: \n")
    query = Lessonplan2017.delete().where(Lessonplan2017.id == lpid)
    query.execute()
    print "9. Back"
    print "0. Quit" 
    choice = raw_input(" >>  ")
...
def writeweekly():
    ...
    lpweeksun = Lessonplan2017.get(Lessonplan2017.week == week)
    ...
    weeksun = Lessonplan2017.select().where(Lessonplan2017.date == datesun)
    ...
    for i in weeksun:
        ...
        if i.theme.startswith("PEPERIKSAAN"):
            ...
            theme = i.theme.upper()
            print >>failkeluar,"\\centerline{%s}" % i.tingkatan
            print >>failkeluar,"\\\\"
            print >>failkeluar,"\n   \\so{%s}\
            \\\\" % theme
            print >>failkeluar,"\n \\textit{%s}  \\\\"\
    ...
    weekmon = Lessonplan2017.select().where(Lessonplan2017.date == datemon)
    ...
    weektue = Lessonplan2017.select().where(Lessonplan2017.date == datetue)
    ...
    weekwed = Lessonplan2017.select().where(Lessonplan2017.date == datewed)
    ...
    weekthu = Lessonplan2017.select().where(Lessonplan2017.date == datethu)
...
def writeweeklyf6():
    ...
    lpweeksun = Lessonplan2017.select().where(Lessonplan2017.week == week)
